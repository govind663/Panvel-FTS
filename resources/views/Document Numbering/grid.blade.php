    ...
    <div class="pd-ltr-20 xs-pd-20-10">
        <div class="min-height-200px">
                    ...
                    <div class="col-md-6 col-sm-12">
                        ...
                        <nav aria-label="breadcrumb" role="navigation">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item "><a >Document Numbering</a></li>
                            </ol>
                        </nav>
                    </div>
                    <div class="col-md-6 col-sm-12 text-right">
                        <a class="btn btn-primary" href="{{ route('document_numbering.create') }}" role="button">
            ...
            <div class="card-box mb-30">
                <div class="pd-20">
                    <h4 class="text-blue h4">Document Numbering List</h4>
                </div>
                <div class="pb-20">
                        ...
                        <tbody>
                            @foreach ($data as $key => $file_type)
                                <tr>
                                    ...
                                    <td><strong>{{ $file_type->company }}</strong></td>
                                    <td>{{ $file_type->document_type }}</td>
                                    <td>{{ $file_type->financial_year }}</td>
                                    <td>{{ $file_type->prefix }}</td>
                                    <td>{{ $file_type->suffix }}</td>
                                    <td>{{ $file_type->Start_Doc_No }}</td>
                                    <td>{{ $file_type->End_Doc_No }}</td>
                                    <td>{{ $file_type->Next_Doc_No }}</td>
                                    ...
                                    @endif
                                    <td>
                                        <a href="{{ route('document_numbering.edit', $file_type->id) }}" class="btn btn-info btn-sm">
                                            <i class="micon dw dw-pencil-1"></i>
                                        </a>
                                    </td>

                                    <td>
                                        <form action="{{ route('document_numbering.destroy', $file_type->id) }}" method="post">
                                            @csrf
                                            @method('DELETE')
